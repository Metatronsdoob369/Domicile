...
export const MIN_FIST_SCORE = 0.91;
 ...
 */
export const fistGuard = {
  async evaluate(stream: KeystrokeSample[]): Promise<number> {
    if (!stream || stream.length === 0) return 0;

    const deltas = stream.map((s) => Math.max(0, s.deltaMs));
    const averageDelta = deltas.reduce((sum, value) => sum + value, 0) / deltas.length;
    const variance =
      deltas.reduce((sum, value) => sum + Math.pow(value - averageDelta, 2), 0) / deltas.length;
    ...
    const pressureScore =
      pressures.reduce((sum, value) => sum + value, 0) / (pressures.length || 1);

    const keys = stream.map((s) => s.key).filter(Boolean) as string[];
    const keyEntropy =
      keys.length === 0
        ? 0.5
        : Math.min(1, new Set(keys).size / Math.max(1, Math.log2(keys.length + 1)));

    const cadenceHash = crypto
      .createHash('sha1')
    ...
    const combined =
      rhythmScore * 0.4 + pressureScore * 0.2 + keyEntropy * 0.2 + cadenceScore * 0.2;

    return Number(Math.min(1, Math.max(0, combined)).toFixed(4));
  },
};

export const KeystrokeOracle: Oracle<BiometricProof> = {
  id: 'fist-62850',
  entropy: 0.84,
  covenantClause: 'All agents MUST present Digital Fist ≥ 91% or be terminated',
  verify: async (session: VoiceSession): Promise<BiometricProof> => {
    const score = await fistGuard.evaluate(session.typingStream);
    return score >= MIN_FIST_SCORE
      ? Proof.Valid(score, session)
      : Proof.Rejected('ANONYMOUS_INVADER', score, session);
  },
};
