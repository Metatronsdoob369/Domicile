...
describe('Orchestrator Core', () => {
  beforeAll(() => {
    vi.spyOn(monitoringDashboard, 'start').mockImplementation(() => {
      // Avoid binding to real ports during unit tests
    });
  });
  ...
  describe('Manifest Compilation', () => {
    it('should compile manifest with basic enhancement areas', async () => {
      ...
      expect(manifest.roadmap?.build_order).toBeDefined();
    });
  ...
  describe('Delegation Execution', () => {
    ...
    it('should execute quantum delegation', async () => {
      ...
      const quantumResults = results.results.quantum as Record<string, any>;
      expect(quantumResults).toBeDefined();
      expect(quantumResults?.backend).toBe('simulator');
    });
    ...
    it('should execute TTS delegation', async () => {
      ...
      const ttsResults = results.results.tts as Record<string, any>;
      expect(ttsResults).toBeDefined();
      expect(ttsResults?.engine).toBe('ECHO-GHOST');
      expect((ttsResults?.segments_processed as number) ?? 0).toBeGreaterThan(0);
    });
    ...
    it('should handle delegation errors gracefully', async () => {
      ...
      await expect(
        orchestrator.executeDelegation(invalidOutput as unknown as AgentOutput)
      ).rejects.toThrow(OrchestratorError);
    });
  });
