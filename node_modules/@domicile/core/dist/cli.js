...
program
    ...
    try {
        ...
        if (options.input) {
            const content = (0, fs_1.readFileSync)(options.input, 'utf-8');
        ...
        (0, fs_1.writeFileSync)(options.output, JSON.stringify(manifest, null, 2));
...
program
    ...
    try {
        ...
        const validation = await orchestrator_1.orchestrator.validateAgentOutputs(Array.isArray(outputs) ? outputs : [outputs]);
        const report = {
...
program
    ...
    try {
        ...
        const results = await orchestrator_1.orchestrator.executeDelegation(agentOutput, {
...
program
    ...
    try {
        ...
        const examples = {
            trend: orchestrator_1.TREND_DETECTION_EXAMPLE,
            quantum: orchestrator_1.QUANTUM_OPTIMIZATION_EXAMPLE,
            voice: orchestrator_1.VOICE_SYNTHESIS_EXAMPLE
        };
        const quantumExamples = {
            qubo: orchestrator_1.DEFAULT_QUBO_EXAMPLE,
            grover: orchestrator_1.DEFAULT_GROVER_ORACLE_EXAMPLE
        };
        if (options.type === 'all') {
            // Write all examples
            Object.entries(examples).forEach(([name, example]) => {
                const filename = (0, path_1.join)(options.output, `${name}_example.json`);
                (0, fs_1.writeFileSync)(filename, JSON.stringify(example, null, 2));
                console.log(`✅ Generated ${filename}`);
            });
            // Write quantum examples
            const quantumFilename = (0, path_1.join)(options.output, 'quantum_examples.json');
            (0, fs_1.writeFileSync)(quantumFilename, JSON.stringify(quantumExamples, null, 2));
            ...
            const filename = (0, path_1.join)(options.output, `${options.type}_example.json`);
            (0, fs_1.writeFileSync)(filename, JSON.stringify(example, null, 2));
            console.log(`✅ Generated ${filename}`);
        }
...
program
    ...
    try {
        ...
        const validation = await orchestrator_1.orchestrator.validateAgentOutputs([orchestrator_1.TREND_DETECTION_EXAMPLE]);
        ...
        try {
            await orchestrator_1.orchestrator.validateAgentOutputs([{ invalid: 'data' }]);
        }
        catch (error) {
...
program
    ...
    try {
        ...
        console.log('===================');
        const securityConfig = orchestrator_1.orchestrator.getSecurityConfig();
        console.log(`🔒 Security: ${securityConfig.rate_limiting.enabled ? 'Enabled' : 'Disabled'}`);
        console.log(`🌲 Pinecone: ${orchestrator_1.orchestrator.isPineconeInitialized() ? 'Initialized' : 'Not initialized'}`);
